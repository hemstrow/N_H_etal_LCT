...
dat2 <- dat[,-which(sample.meta(dat)$LCT < .99)]

# save filtering info
n_before <- table(paste0(sample.meta(dat)[,"stream"], ".", sample.meta(dat)[,"year"]))
n_after <- table(paste0(sample.meta(dat2)[,"stream"], ".", sample.meta(dat2)[,"year"]))
n <- data.frame(subfacet = names(n_before), n_before = as.numeric(n_before), n_after = 0)
n$n_after <- n_after[match(n$subfacet, names(n_after))]
n$n_after[is.na(n$n_after)] <- 0


percent_hyb_before <- as.data.table(sample.meta(dat))
percent_hyb_before[,subfacet := paste0(stream, ".", year)]

percent_hyb_after <- as.data.table(sample.meta(dat2))
percent_hyb_after[,subfacet := paste0(stream, ".", year)]


for(i in c("LCT", "YCT", "RBT")){
  percent_hyb_before2 <- percent_hyb_before[, mean(get(i)), by = subfacet]
  percent_hyb_after2 <- percent_hyb_after[, mean(get(i)), by = subfacet]
  percent_hyb2 <- merge(percent_hyb_before2, percent_hyb_after2, by = "subfacet", all = TRUE)
  colnames(percent_hyb2) <- c("subfacet", paste0("percent_", i, c("_before", "_after")))
  if(!exists("percent_hyb")){
    percent_hyb <- percent_hyb2
  }
  else{
    percent_hyb <- merge(percent_hyb, percent_hyb2, by = "subfacet", all = TRUE)
  }
...
# hyb percents
stats <- merge(stats, percent_hyb, by = "subfacet", all = TRUE)
...
# basic correlations:
## melt data
# year.matches <- which(abs(stats$year  - stats$PVA.Abund.Year) <= 5 & stats$n_after >= 5)
year.matches <- which(!is.na(stats$PVA.Abund.Year) & stats$n_after >= 5)
...
# combine
colnames(models) <- c("Statistic", "Extinction", "Harmonic_Mean_N", "Model Type", "Adjusted Extinction", "Adjusted Harmonic Mean N")
colnames(Table.S3) <- c("Statistic", "Extinction", "Harmonic_Mean_N", "Model Type",
